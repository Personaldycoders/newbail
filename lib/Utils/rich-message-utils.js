...
// Dycoders@Changes 09-04-26 --- Inject buffer into unifiedResponse.data to support proper rendering of rich messages (ex: tables and code blocks)
...
export const prepareRichResponseMessage = (content) => {
    ...
    const richResponseMessage = proto.AIRichResponseMessage.create({
        ...
        unifiedResponse: {
            data: Buffer.from(JSON.stringify(unified), 'utf-8') // Dycoders@Note 25-04-26 --- Expects "ArrayBufferLike"
        },
        contextInfo: {
            isForwarded: true,
            forwardingScore: 1,
    ...
    // Dycoders@Note 13-05-26 --- Add disclaimer text on richResponseMessage
    if (disclaimerText) {
        botMetadata.messageDisclaimerText = disclaimerText;
    }
    // Dycoders@Note 15-05-26 --- Add responseId from unified directly to botMetadata
    botMetadata.botResponseId = unified.response_id;
    return message;
};
// Dycoders@Note 17-04-26 --- signature and certificateChain for proofs[] field
...
export const wrapToBotForwardedMessage = (richResponseMessage) => ({
    messageContextInfo: {
        ...
        botMetadata: {
            pluginMetadata: {},
            // Dycoders@Note 09-04-26 --- TODO: Fill verificationMetadata field
